...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if node_a.prenodes.issubset(self.s_levels[level]):
                # use instances of states from "self.s_levels[level]", not from "node_a.prenodes"
                for node_s in self.s_levels[level]:
                    if node_s in node_a.prenodes:
                        # add child for a state node
                        node_s.children.add(node_a)
                        # add parent for an action node
                        node_a.parents.add(node_s)

                nodes_a.add(node_a)

        # create a level of action nodes
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
            for node_s in node_a.effnodes:
                # create new instance of a state node
                node_s = PgNode_s(node_s.symbol, node_s.is_pos)
                # add child for an action node
                node_a.children.add(node_s)
                # add parent for a state node
                node_s.parents.add(node_a)
                # add node to the set
                nodes_s.add(node_s)

        # create a level of state nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for node1, node2 in [(node_a1, node_a2), (node_a2, node_a1)]:
            for e_add in node1.action.effect_add:
                if e_add in node2.action.effect_rem:
                    check = True
                    break
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for node1, node2 in [(node_a1, node_a2), (node_a2, node_a1)]:
            for e_add in node1.action.effect_add:
                if e_add in node2.action.precond_neg:
                    check = True
                    break

            if not check:
                for e_rem in node1.action.effect_rem:
                    if e_rem in node2.action.precond_pos:
                        check = True
                        break
    ...
    def h_levelsum(self) -> int:
        ...
        for literal in self.problem.goal:
            for i, s_level in enumerate(self.s_levels):
                if PgNode_s(literal, True) in s_level:
                    level_sum += i
                    break
